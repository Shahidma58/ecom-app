...
export default function ProdEnquiry() {
  ...
  // Applied branch code (only updated on button click)
  const [appliedBrnCode, setAppliedBrnCode] = useState("");
  ...
  const fetchData = async () => {
    const params = new URLSearchParams({
      ...
      brn_code: appliedBrnCode, // Use appliedBrnCode instead of brnCode
    });

    console.log('Applied Branch Code:', appliedBrnCode);
  ...
  // Fetch when entering page or when dependencies change
  useEffect(() => {
    console.log('Fetching data...')
    fetchData();
  }, [page, debouncedDesc, debouncedBrand, sortBy, sortOrder, appliedBrnCode]); 
  // Note: appliedBrnCode in dependency, not brnCode
  ...
  // Handle Display button click - applies the branch filter
  const handleDisplayClick = () => {
    setAppliedBrnCode(brnCode); // Apply the current brnCode
      ...
      <div className="grid grid-cols-3 gap-4 bg-white p-1 rounded-lg shadow mb-1 border border-emerald-200">
        ...
        <div className="flex gap-2">
          <input
            ...
            maxLength={3}
            placeholder="Branch"
            className="w-20 p-1 border rounded-lg focus:ring-emerald-500 border-emerald-300"
            value={brnCode}
          ...
          <button 
            className="w-20 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-1.5 px-2 rounded-md shadow-md transition-all"
